...
DIST_NAME := winrun
DIST_VERSION := $(shell grep VERSION src/WinRun.pro | head -1 | cut -d'=' -f2 | awk '{print $$1}' | sed 's/"//g')
...
SOURCE_LIST := Makefile CHANGELOG.md CONTRIBUTING.md LICENSE.md README.md src/ docs/
...
install: bin/winrun
	...
	@sudo install bin/winrun $(DESTDIR)/$(PREFIX)/bin/
	@mkdir -p build/man/
	...
	@gzip -cn --best build/man/winrun.1 > build/man/winrun.1.gz
	@sudo install -m 644 build/man/winrun.1.gz /usr/share/man/man1/
	@sudo mandb -q
	@echo Installed at $(DESTDIR)/$(PREFIX)/bin/ | sed 's^//^/^g'

uninstall: $(DESTDIR)/$(PREFIX)/bin/winrun
	@sudo $(RM) $(DESTDIR)/$(PREFIX)/bin/winrun
	@sudo $(RM) /usr/share/man/man1/winrun.1.gz
	@sudo mandb -q
...
release: src/WinRun.pro
	...
	@cd build/release ; qmake -qt=qt5 CONFIG+=release ../../src/WinRun.pro ; make
	@mkdir -p bin/
	@cp build/release/winrun bin/winrun

debug: src/WinRun.pro
	$(if $(findstring 0,$(HAS_QMAKE)),,$(error No 'qmake' in path, consider installing 'qtbase5-dev' package))
	$(if $(findstring 0,$(HAS_X11EXTRAS)),,$(error X11 extras not found, consider installing 'libqt5x11extras5-dev' package))
	-@rm -r build/debug 2>/dev/null || true
	@mkdir -p build/debug
	@cd build/debug ; qmake -qt=qt5 CONFIG+=debug ../../src/WinRun.pro ; make
	@mkdir -p bin/
	@cp build/debug/winrun bin/winrun
...
test: src/WinRun.pro
	...
	@mkdir -p build/test
	@cd build/test ; qmake -qt=qt5 CONFIG+=test ../../src/WinRun.pro ; make
	@build/test/winruntests \
...
package: clean dist
	...
	@cp src/icons/16x16/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/16x16/apps/winrun.png
	@cp src/icons/24x24/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/24x24/apps/winrun.png
	@cp src/icons/32x32/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/32x32/apps/winrun.png
	@cp src/icons/48x48/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/48x48/apps/winrun.png
	@cp src/icons/64x64/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/64x64/apps/winrun.png
	@cp src/icons/96x96/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/96x96/apps/winrun.png
	@cp src/icons/128x128/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/128x128/apps/winrun.png
	@cp src/icons/192x192/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/192x192/apps/winrun.png
	@cp src/icons/256x256/app.png $(PACKAGE_DIR)/usr/share/icons/hicolor/256x256/apps/winrun.png
	...
	@mkdir -p $(PACKAGE_DIR)/usr/share/doc/winrun/
	@cp package/deb/copyright $(PACKAGE_DIR)/usr/share/doc/winrun/copyright
	@cp CHANGELOG.md build/changelog
	@sed -i '/^$$/d' build/changelog
	@sed -i '/## Release Notes ##/d' build/changelog
	@sed -i '1{s/### \(.*\) \[.*/winrun \(\1\) stable; urgency=low/}' build/changelog
	@sed -i '/###/,$$d' build/changelog
	@sed -i 's/\* \(.*\)/  \* \1/' build/changelog
	@echo >> build/changelog
	@echo ' -- Josip Medved <jmedved@jmedved.com>  $(shell date -R)' >> build/changelog
	@gzip -cn --best build/changelog > $(PACKAGE_DIR)/usr/share/doc/winrun/changelog.gz
	@mkdir -p build/man/
	@sed 's/MAJOR.MINOR.PATCH/$(DIST_VERSION)/g' docs/man/winrun.1 > build/man/winrun.1
	@sed -i 's/CURR_DATE/$(MAN_DATE)/g' build/man/winrun.1
	...
	@install -d $(PACKAGE_DIR)/opt/winrun/
	@install bin/winrun $(PACKAGE_DIR)/opt/winrun/
	@strip $(PACKAGE_DIR)/opt/winrun/winrun
...
preview: docs/man/winrun.1
	@mkdir -p build/man/
	@sed 's/MAJOR.MINOR.PATCH/$(DIST_VERSION)/g' docs/man/winrun.1 > build/man/winrun.1
	@sed -i 's/CURR_DATE/$(MAN_DATE)/g' build/man/winrun.1
	@man -l build/man/winrun.1